...
// v2 protocol frame constants - Protocol: v2 - 支持未来升级到v3
const (
    FRAME_MAGIC   = 0xA5
    FRAME_VERSION = 2
    CMD_AUTH      = 0x01
...
func handleTunnel(conn net.Conn, target, clientAddr string, mode int, firstFrame string) error {
    ...
    mu.Lock()
    err = wsConn.WriteMessage(websocket.BinaryMessage, authFrame)
    mu.Unlock()
    if err != nil {
        sendErrorResponse(conn, mode)
    ...
    openFrame := makeFrame(CMD_OPEN, 0x00, openPayload)
    mu.Lock()
    err = wsConn.WriteMessage(websocket.BinaryMessage, openFrame)
    mu.Unlock()
    if err != nil {
        sendErrorResponse(conn, mode)
        return fmt.Errorf("发送OPEN帧失败: %w", err)
    }
    ...
    if cmd == CMD_CLOSE {
        log.Printf("[代理] %s 连接失败: %s", clientAddr, string(payload))
        sendErrorResponse(conn, mode)
        return fmt.Errorf("连接失败: %s", string(payload))
    }
    ...
    go func() {
        ...
        for {
            ...
            if err != nil {
                mu.Lock()
                wsConn.WriteMessage(websocket.BinaryMessage, makeFrame(CMD_CLOSE, 0x00, nil))
                mu.Unlock()
                done <- true
                return
            }
            ...
            mu.Lock()
            err = wsConn.WriteMessage(websocket.BinaryMessage, makeFrame(CMD_DATA, 0x00, buf[:n]))
            mu.Unlock()
            if err != nil {
                done <- true
                return
